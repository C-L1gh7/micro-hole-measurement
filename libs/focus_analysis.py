...
def get_best_focus_images(processed_path=None, method='laplacian', verbose=True):
    """
    获取最佳聚焦度的图片（简化的接口函数）
...
# 简化的调用函数
def analyze_focus(processed_path=None, method='laplacian', verbose=True, save_file=True):
    """
    一键分析聚焦度（最简化的接口）
    ...
    Returns:
        dict: 分析结果
    """
    result = get_best_focus_images(processed_path, method, verbose)
